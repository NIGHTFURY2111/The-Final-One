...
// Modified for EC_Component usage
...
public class ExtendedScriptableObjectDrawer : PropertyDrawer
{
    ...
    public override float GetPropertyHeight(SerializedProperty property, GUIContent label)
    {
        ...
        // Don't show inline editing for certain types
        if (!ShouldDrawInline(property))
            return totalHeight;

        string foldoutKey = GetFoldoutKey(property);
        bool isExpanded = foldoutStates.ContainsKey(foldoutKey) && foldoutStates[foldoutKey];
    ...
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
    {
        ...
        if (property.objectReferenceValue != null)
        {
            ...
            foldoutStates[foldoutKey] = isExpanded;
        }
        else
        {
        ...
        if (property.objectReferenceValue == null) propertyRect.width -= buttonWidth;
    ...
    {
        // Only draw for AC_Component and its derivatives
        if (property.objectReferenceValue == null) return true;
        
        var obj = property.objectReferenceValue;
        var type = obj.GetType();
        
        // Check if it's an AC_Component or derived type
        var baseType = type;
        while (baseType != null)
        {
            if (baseType.Name == "AC_Component")
                return true;
            baseType = baseType.BaseType;
        }
        
        // For now, show all ScriptableObjects except ignored ones
        return true;
    }
